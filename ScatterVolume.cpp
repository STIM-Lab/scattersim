...
void InitMatrices() {
	A = Eigen::MatrixXcd::Zero(6 * L, 6 * L);									// allocate space for the matrix
	b = Eigen::VectorXcd::Zero(6 * L);												// zero out the matrix
}
...
// pre-compute and store all of the Sz values
...
/// This system uses two boundary conditions:
//		1) The incoming waves in the top layer are defined by the incident field
//		2) There are no reflected waves within the bottom layer
...
int main(int argc, char** argv) {
	...
	// Most of this code is just a safety net to deal with users being lazy. Estimates the number of layers that the user
	// would like to simulate based on their input for other values.
	// 
	...
	InitMatrices();											// generate the matrix A, x, and b
	InitLayerProperties();									// store all of the layer positions and refractive indices

	
	unsigned int N[2];										// calculate the number of samples for large lenses (this is the number of plane waves in the incident field)
	...
	// if the user asks for a lens of zero size (NA = 0), then only simulate one plane wave
	if (in_alpha == 0 || N[0] * N[1] == 1) {
		I.push_back(i_ref);
	}
	// otherwise, generate a bunch of plane waves to fill the specified solid angle representing the lens
	...
	cw.Layers.resize(L - 1);											// resize the coupled wave structure to match the number of layers

	for (size_t p = 0; p < I.size(); p++) {								// for each incident plane wave ("sample")
		tira::planewave<double> i = I[p];								// store the incident plane wave locally
		InitMatrices();
		InitLayerProperties();
		InitSz(i);														// initialize the model matrix with all of the necessary conditions
		SetBoundaryConditions(i);
		SetGaussianConstraints(i);
		SetTransverseConstraints(i);

		// At this point, the matrix A can be solved to calculate the reflected and transmitted waves at each layer
		Eigen::VectorXcd x = A.colPivHouseholderQr().solve(b);			// solve the linear system
		std::vector<tira::planewave<double>> P = mat2waves(i, x);		// convert the solution vector to a list of plane waves
		cw.Pi.push_back(i);														// push the incident plane wave (used in this simulation) to Pi in the coupled wave structure

		// push all of the other waves into the coupled wave structure
		for (size_t l = 0; l < L - 1; l++) {														// for each layer
			cw.Layers[l].z = z[l];
	...
	if (in_outfile != "") {									// save the output file if one is specified (it should be by default)
		cw.save(in_outfile);
	}

	// This section doesn't impact the results, but instead outputs a "reference wave" 
	// to the console so that the user can verify that the program worked as expected
	InitSz(i_ref);
