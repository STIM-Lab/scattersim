...
num_pixels = [100, 100, 100]     # shape would be (8, 4, 50)
n_layers = [1, 1]
n_medium = 1.0
n_sample = 1.4+0.07j
center = [0, 0, 0]
...
def RecBar():
    ...
    index[int((center[1]-width/2 - pos_interest[2])/(pos_interest[3]-pos_interest[2]) * num_pixels[1]) :
    int((center[1]+width/2 - pos_interest[2])/(pos_interest[3]-pos_interest[2]) * num_pixels[1]),
    int((center[0]-width/2 - pos_interest[0])/(pos_interest[1]-pos_interest[0]) * num_pixels[0]) :
    int((center[0]+width/2 - pos_interest[0])/(pos_interest[1]-pos_interest[0]) * num_pixels[0]),
    int((center[2]-height/2 - pos_interest[4])/(pos_interest[5]-pos_interest[4]) * num_pixels[2]):
    int((center[2]+height/2 - pos_interest[4])/(pos_interest[5]-pos_interest[4]) * num_pixels[2])] = n_sample
...
samplew = RecBar()
sample = np.asfortranarray(samplew[:, :, 50])
import matplotlib.pyplot as plt
plt.imshow(np.real(sample))
plt.show()

outFile = "C:\\Users\\sunrj\\Dropbox\\scatter\\data\\RecSquare.npy"
